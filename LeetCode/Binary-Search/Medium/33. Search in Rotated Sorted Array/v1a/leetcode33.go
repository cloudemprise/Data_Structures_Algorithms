...
func search(nums []int, target int) int {
	...
	lo, hi := 0, len(nums)-1
	if target <= nums[len(nums)-1] { // target value in upper split
		lo = split
	} else {
		hi = split - 1 // target value in lower split
	}

	// 3. Perform a target value binary search.
	modNums := nums[lo : hi+1]
	modIndex := binarySearch(modNums, target)
	if modIndex == -1 {
		return modIndex
	}
	return lo + modIndex
