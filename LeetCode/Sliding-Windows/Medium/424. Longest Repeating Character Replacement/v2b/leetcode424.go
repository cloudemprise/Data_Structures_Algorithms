...
// Sliding Window Technique using a fix array size to remember the count of each character.
...
func characterReplacement(s string, k int) int {
	...
	// Keep track of the maximum character count...
	highestCount := 0
	// by recalling the count of each character.
	charCount := make([]int, 26) // O(1)
	...
	for right := 0; right < len(s); right++ {
		...
		highestCount = max(highestCount, charCount[s[right]-'A'])
		...
		if (right-left+1)-(highestCount+k) > 0 { // If so...
			charCount[s[left]-'A']-- // remove character from consideration...
