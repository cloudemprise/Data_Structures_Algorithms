...
// Sliding Window Technique by remembering the character with the highest frequency count.
...
// and fails for: maxLength - maxCount - k > 0, thus reduce window-span for such
// occasion.
...
func characterReplacement(s string, k int) int {
	...
	// Keep track of the highest character...
	highestChar := byte(0)
	...
	for right := 0; right < len(s); right++ {
		...
		charCount[s[right]]++
		// and check if it is the highest character count.
		if highestChar == 0 || charCount[s[right]] > charCount[highestChar] {
			highestChar = s[right]
		}

		// If the window-span minus the count of the dominate character is larger
		// than k (means we cannot swap all different characters) then shrink the
		// window-span.
		if (right-left+1)-charCount[highestChar] > k {
