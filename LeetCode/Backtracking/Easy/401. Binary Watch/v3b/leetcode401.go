...
// Solution 3b: Backtracking recusive closure with separate hrs and mins variables.
	...
	backtrack = func(hrs, mins, numOfOnLEDs, skipDuplicates int) {
		...
		for LEDindex := skipDuplicates; LEDindex < totalLEDs; LEDindex++ {
			...
			if LEDindex >= 6 {
				...
				if hrs+(1<<(LEDindex-6)) >= 12 {
					return // base case.
				}
				// Search for the next solution.
				backtrack(hrs+(1<<(LEDindex-6)), mins, numOfOnLEDs+1, LEDindex+1)
				...
				// Ignore invalid solutions.
				if mins+(1<<LEDindex) >= 60 {
					return // base case.
				}
				// Search for the next solution.
				backtrack(hrs, mins+(1<<LEDindex), numOfOnLEDs+1, LEDindex+1)
			}
		}
	}
