...
// Solution 1. Backtracking recursive closure.
func subsetXORSum(nums []int) (sum int) {

	// Define a func variable.
	var genSubsets func([]int)
	// Define recursive clousure subset search.
	genSubsets = func(subset []int) {
		if len(subset) == len(nums) { // base case.
			var xOR int // local accumulator.
			for i, val := range subset {
				// Business logic.
				xOR ^= nums[i] * val // 0/1
			}
			// Accumulate subset xOR.
			sum += xOR
		} else {
