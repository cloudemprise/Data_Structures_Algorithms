...
func carFleet(target int, position []int, speed []int) int {
	...
	// Use the Decreasing Monotonic Stack algorithm:
	previousCars := make([]car, 0)
	fleet := 0
	for i, current := range cars {

		// If the car behind requires more time ('duration') than the car in
		// front then no hold-up occurs and the number of arriving groups of car,
		// i.e. fleet number, goes up by one.
		if (i > 0) && (current.duration > previousCars[len(previousCars)-1].duration) {
			fleet++ // No hold up thus increment fleet.
			previousCars = previousCars[:len(previousCars)-1]
		}
		previousCars = append(previousCars, current) // Keep track of the next slowest car.
	} // Time Complexity: O(n)

	return fleet + 1 // +1 to account for the 1st car (initial state)
}
