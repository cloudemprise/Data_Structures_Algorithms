...
func copyRandomList(head *Node) *Node {
	...
	// Creating a mapping between random pointer location and their
	// destinations.
	mapping := make(map[int]int)
	for i, n := range oldStack {
	...
	// Allocate that mapping to the newStack random pointers.
	for i, n := range oldStack {
		if n.Random == nil {
			newStack[i].Random = nil
		} else {
			newStack[i].Random = newStack[mapping[i]]
		}
	}
