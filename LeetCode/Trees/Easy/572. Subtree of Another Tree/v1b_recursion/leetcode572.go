...
/*
The isSubtree function takes two pointers to TreeNode structs as input, root and
subRoot, and returns a boolean indicating whether subRoot is a subtree of root.

The function first checks if root or subRoot is nil, in which case it returns
true if both are nil and false if only one of them is nil. If root and subRoot
are the same tree, it returns true. Otherwise, it recursively checks if subRoot
is a subtree of the left or right subtree of root. The isSameTree function is a
helper function that checks if two trees are the same. It returns true if both
trees are nil, false if only one of them is nil, and recursively checks if the
values and subtrees of the two trees are the same.
...
func isSameTree(s *tree.TreeNode, t *tree.TreeNode) bool {
	// Short-hand, structural & uncorrelated check.
	if s == nil || t == nil {
		return s == t
	}
