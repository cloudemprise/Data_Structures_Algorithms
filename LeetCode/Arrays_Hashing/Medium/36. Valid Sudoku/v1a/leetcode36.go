...
func isValidSudoku(board [][]byte) bool {
	...
	for row := 0; row < 9; row++ {
		...
		horizontal := make(map[int]struct{})
		vertical := make(map[int]struct{})
		...
		for col := 0; col < 9; col++ {
			...
			// Check 1: horizontal
			cell = string(board[row][col]) // Read particular site.
			...
			// Check 2: vertical.
			cell = string(board[col][row]) // Read particular site.
			// Only process valid digits, ignore all else.
			if digit, err := strconv.Atoi(cell); err == nil {
				// Check if value is a duplicate.
			...
			cell = string(board[r][c]) // Read particular site.
			// Only process valid digits, ignore all else.
			if digit, err := strconv.Atoi(cell); err == nil {
				// Check if value is a duplicate.
