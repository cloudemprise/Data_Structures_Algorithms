...
import (
	...
	"github.com/cloudemprise/binary-search_breath-first/queue"
)
...
// insertNode recursively searches for the next available insertion slot of a
// binary search tree.
func (n *Node) insertNode(d int) {
	...
	switch {
	case d <= n.Data: // go left
		if n.Left == nil {
			n.Left = newNode(d)
		} else {
			n.Left.insertNode(d)
		}
	case d > n.Data: // go right
		if n.Right == nil {
			n.Right = newNode(d)
		} else {
			n.Right.insertNode(d)
		}
	}
}
